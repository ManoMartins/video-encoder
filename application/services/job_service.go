...
func (j *JobService) performUpload() error {
	...
	videoUploadService := NewVideoUploadService()
	videoUploadService.OutputBucket = os.Getenv("OUTPUT_BUCKET_NAME")
	videoUploadService.VideoPath = os.Getenv("LOCAL_STORAGE_PATH") + "/" + j.VideoService.Video.ID
	concurrency, _ := strconv.Atoi(os.Getenv("CONCURRENCY_UPLOAD"))
	doneUpload := make(chan string)

	go videoUploadService.ProcessUpload(concurrency, doneUpload)
